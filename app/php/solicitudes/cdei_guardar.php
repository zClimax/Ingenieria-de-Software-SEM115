...
$sid   = (int)($_POST['id'] ?? 0);
$idReg = (int)($_POST['id_edi_diplomado'] ?? 0);
...
if (strtoupper((string)$S['TIPO_DOCUMENTO']) !== 'CDEI') {
  http_response_code(403);
  exit('La solicitud no es de tipo CDEI');
}
...
if ($nombreModulo === '' || $horasImpartidas <= 0) {
  header('Location: /siged/public/index.php?action=jefe_ver&id='.$sid.'&cdei_err=campos');
  exit;
}

// Upsert
$exSt = $pdo->prepare("
  SELECT ID_EDI_DIPLOMADO
...
if ($exId > 0) {
  $up = $pdo->prepare("
    UPDATE dbo.DOCENTE_DIPLOMADO_EDUCACION_INCLUSIVA
  ...
  $ins = $pdo->prepare("
    INSERT INTO dbo.DOCENTE_DIPLOMADO_EDUCACION_INCLUSIVA
...
header('Location: /siged/public/index.php?action=jefe_ver&id='.$sid.'&cdei_saved=1');
exit;
