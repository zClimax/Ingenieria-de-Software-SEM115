...
$idUser = (int)($u['id'] ?? 0);
if ($idUser <= 0) { http_response_code(403); echo "Sesión inválida"; exit; }

try { $pdo = DB::conn(); }
catch (Throwable $e) { http_response_code(500); echo "Sin conexión a BD"; exit; }
...
if ($mime === 'image/png') {
  ...
  if (!$src) { header('Location: /siged/public/index.php?action=home_docente&msg=firma_error'); exit; }
...
try {
  ...
  $st2->execute([':ruta'=>$relWeb, ':mime'=>'image/png', ':id'=>$idUser]);
}
